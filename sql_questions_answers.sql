...
-- 1) Select all products with their category name.
SELECT 
    products.ProductID,
...
-- 2) List all customers from USA or Mexico.
SELECT 
    *
FROM
    customers
...
SELECT 
    customers.CustomerName, COUNT(orders.OrderID) AS TotalOrders
FROM
    orders
        INNER JOIN
...
-- 4) Find the top 5 products with the highest unit price.
SELECT 
...
-- 5) Get a list of all employees born before 1960.
SELECT 
...
-- 7) Show the total sales grouped by product.
SELECT 
    products.ProductID,
    products.ProductName,
...
-- 8) List suppliers along with the number of products they supply.
SELECT 
...
-- 9) Display orders with shipper name and customer name.
SELECT 
    orders.OrderID,
...
-- 10) Find customers who placed more than 5 orders.
SELECT 
